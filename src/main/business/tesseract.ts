    ...
    async on() {
        ...
        for (let index = 0; index < workerLen; index++) {
            const worker = createWorker({
                langPath: resourcesPathGet('root', 'tesseract/langPath'),
                cachePath: resourcesPathGet('root', 'tesseract/cachePath'),
                // logger: m => logInfo('[tesseract info]', m),
                errorHandler: err => logWrapper('error', undefined, '[tesseract error]', err)
            })
